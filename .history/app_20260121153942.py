...
def handle_multi_item_text(msg_raw, s, MENU, lang):
    ...
    # ✅ FOOD GENERICS FIRST → ASK LIST (meals/juices/drinks/sides)
    food_only = [
        g for g in (generics or [])
        if g.get("kind") in ("meals", "juices", "drinks", "snacks_sides")
    ]
    
    # ✅ GENERIC burger/sandwich/wrap
    bs_only = [
        g for g in (generics or [])
        if (g.get("kind") in ("burger", "sandwich"))
    ]

    # Combine ALL into generic_queue at once
    # Ordering: Food first (simpler), then Burgers/Sandwiches (complex)
    # OR: The order the user asked? 
    # Let's preserve user order if possible, but here we separated them.
    # Let's put food_only first, then bs_only.
    
    if food_only or bs_only:
        s.setdefault("generic_queue", [])
        # We append, but be careful not to double add if we called this function recursively?
        # Actually this function is entry point. We should extend.
        for g in food_only:
            s["generic_queue"].append(g)
        for g in bs_only:
            s["generic_queue"].append(g)
            
        # NOW process the first item in the queue
    ...
    # If NO generics but we have specific spice queue
    ...
    if s.get("spice_queue"):
        ...
        # Calculate prefix again if needed (or reuse if available)
        prefix = (
            ("تمت إضافة: " + "، ".join(added_lines) + "<br><br>")
            if added_lines and lang == "ar"
            else ("Added: " + ", ".join(added_lines) + "<br><br>" if added_lines else "")
...
def parse_intelligent_order(msg: str, lang: str = "en") -> dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            "2. If user mentions SPECIFIC item name (partial or full) matches menu => type=\"specific\"\n"
            "   (e.g. \"chicken\" -> \"Chicken Burger\", \"beef\" -> \"Beef Burger\", \"pepsi\" -> \"Pepsi\")\n"
            "3. MATCHING RULE: If input is \"2 chicken\" and menu has \"Chicken Burger\", return SPECIFIC item \"Chicken Burger\", NOT generic.\n"
            "4. Handle typos intelligently: cofee→Coffee, burgur→Burger, sandwihc→Sandwich\n"
            "5. Extract quantities: \"2 chicken and 3 beef\" → {\"type\":\"specific\", \"name\":\"Chicken Burger\", \"qty\":2}, {\"type\":\"specific\", \"name\":\"Beef Burger\", \"qty\":3}\n"
            "6. Detect preferences: spicy/non-spicy for burgers\n\n"
            "OUTPUT (JSON ONLY):\n"
            "{\n"
            "  \"items\": [\n"
            "    {\"type\": \"generic\", \"category\": \"burger\", \"qty\": 2},\n"
        ...
        for it in items:
            ...
            if item_type == "generic":
                ...
                if category in ["burger", "burgers"]:
                    category = "burger"
                elif category in ["sandwich", "sandwiches", "wrap", "wraps", "tortilla"]:
                    category = "sandwich"
                    ...
                    category = "drinks"
                elif category in ["snack", "snacks", "side", "sides"]:
...
def make_chat_response(reply, lang, menu=None, buttons=None):
...
def chat():
    ...
    if s.get("stage") == "payment" and (
        ...
        if lang == "ar":
            reply = "كيف ترغب في الدفع؟"
            buttons = [
                {"label": "نقدي (Cash)", "value": "نقدا"},
                {"label": "دفع إلكتروني (Online)", "value": "دفع إلكتروني"}
            ]
        else:
            reply = "How would you like to pay?"
            buttons = [
                {"label": "Cash", "value": "Cash"},
                {"label": "Online Payment", "value": "Online Payment"}
            ]

        return make_chat_response(reply, lang, buttons=buttons)
