...
def whatsapp_webhook():
        # --- Detect WhatsApp chat clear (no context/session) ---
        # If WhatsApp_SESSIONS has no state for this user, treat as new session
        if user_number not in WHATSAPP_SESSIONS or not WHATSAPP_SESSIONS[user_number].get("state"):
            WHATSAPP_SESSIONS[user_number] = {
                "state": {"stage": None, "order": [], "total": 0, "last_item": None, "last_qty": 0, "last_confirmed_item": None, "pending_item": None, "spice_queue": [], "generic_queue": []},
                "messages": [],
                "lang": "en",
            }
            send_whatsapp_text(user_number, "New order started!\n\nYour previous session was cleared. What would you like to order?")
            # Continue to process the new message as a new order
    # VERIFY (GET)
    ...
    # --- After payment (cash/online), mark order as complete and require new order for next ---
    # Look for payment confirmation keywords and reset session
    payment_keywords = ["payment done", "paid", "cash paid", "online paid", "order complete", "تم الدفع", "تم السداد", "الدفع اونلاين", "الدفع نقداً", "اكتمل الطلب"]
    if any(kw in user_text.lower() for kw in payment_keywords):
        WHATSAPP_SESSIONS[user_number] = {
            "state": {"stage": None, "order": [], "total": 0, "last_item": None, "last_qty": 0, "last_confirmed_item": None, "pending_item": None, "spice_queue": [], "generic_queue": []},
            "messages": [],
            "lang": lang,
        ...
        reply = (
            "تم إكمال الطلب بنجاح!\n\nإذا كنت ترغب في طلب جديد، يرجى كتابة 'طلب جديد' أو 'ابدأ من جديد'."
            if lang == "ar" else
            "Order completed successfully!\n\nIf you want to place a new order, please type 'new order' or 'start new order'."
        )
        send_whatsapp_text(user_number, reply)
        return "ok", 200
