...
def whatsapp_webhook():
        # --- SESSION RESET ON CHAT CLEAR (WhatsApp) ---
        # WhatsApp does not send a special event for chat clear, but if the user sends a message and there is no session, treat as new order
        if user_number not in WHATSAPP_SESSIONS or not WHATSAPP_SESSIONS[user_number].get("state"):
            ...
            send_whatsapp_text(user_number, "New order started! 🎉\n\nWhat would you like to order?" if (stored_lang or "en") == "en" else "تم بدء طلب جديد! 🎉\n\nماذا تريد أن تطلب؟")
            # Do not return here, let the message process as normal
    # VERIFY (GET)
    ...
    # --- SESSION RESET AFTER PAYMENT ---
    # If the order is marked as paid (cash or online), clear the session for this user so next message starts a new order
    ctx_state = WHATSAPP_SESSIONS.get(user_number, {}).get("state", {})
    if ctx_state.get("stage") in ("payment_done", "order_complete", "paid", "finished") or ctx_state.get("order_complete_flag"):
        WHATSAPP_SESSIONS[user_number] = {
            "state": {"stage": None, "order": [], "total": 0, "last_item": None, "last_qty": 0, "last_confirmed_item": None, "pending_item": None, "spice_queue": [], "generic_queue": []},
            "messages": [],
            "lang": lang,
        }
        send_whatsapp_text(user_number, "New order started! 🎉\n\nWhat would you like to order?" if lang == "en" else "تم بدء طلب جديد! 🎉\n\nماذا تريد أن تطلب؟")
        # Do not return here, let the message process as normal
