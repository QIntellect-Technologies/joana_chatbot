...
def chat():
    ...
    # ✅ NEW ORDER REQUEST - Allow user to start fresh order at any stage
    new_order_keywords = [
        "new order", "start new order", "another order", "add one more order", "start again",
        "طلب جديد", "طلب آخر", "أبدأ من جديد", "طلب جديد من فضلك"
    ]
    if any(keyword in msg_l for keyword in new_order_keywords):
        # Reset session completely
        session["state"] = {"stage": None, "order": [], "total": 0, "last_item": None, "last_qty": 0, 
                           "last_confirmed_item": None, "pending_item": None, "spice_queue": [], "generic_queue": []}
        reply = (
            "تم بدء طلب جديد! 🎉<br>كيف يمكنني مساعدتك اليوم؟"
            if lang == "ar" else
            "New order started! 🎉<br>How can I help you today?"
        )
        return make_chat_response(reply, lang)
