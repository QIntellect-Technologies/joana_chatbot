...
def chat():
    ...
    # ✅ CRITICAL FIX: For Arabic at add_more OR initial stage, try intelligent parsing BEFORE conversational fallback
    # This catches cases like "أريد قهوتين" (I want 2 coffees) that don't match simple patterns
    # English already works because find_menu_item() is more lenient, but Arabic needs this
    # Allow at stage=None so users can start ordering in Arabic without clicking buttons first
    if lang == "ar" and s.get("stage") in (None, "add_more") and not is_cancel_text(msg):
