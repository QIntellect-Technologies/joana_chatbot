...
def handle_multi_item_text(msg_raw, s, MENU, lang):
    ...
    for spec in specific_items:
        ...
        # Track satisfied category
        satisfied_counts[cat] = satisfied_counts.get(cat, 0) + qty

    # ✅ DEDUCT SATISFIED QUANTITIES FROM GENERICS
    # Consumes specific items from the generic request to avoid repeated questions
    final_generics = []
    for g in generics:
        g_kind = (g.get("kind") or "").strip().lower()
        g_qty = int(g.get("qty") or 1)
        
        canon_g = canonical_category(g_kind)
        
        if satisfied_counts.get(canon_g, 0) > 0:
            deducted = min(g_qty, satisfied_counts[canon_g])
            g_qty -= deducted
            satisfied_counts[canon_g] -= deducted
        
        if g_qty > 0:
            g["qty"] = g_qty
            final_generics.append(g)
...
def chat():
    ...
    # 2️⃣.5️⃣ CHECKOUT / COMPLETE ORDER
    elif s.get("order") and any(k in msg_txt.lower() for k in ["place order", "place the order", "complete", "finish", "checkout", "check out", "payment", "pay", "done", "proceed"]):
        s["stage"] = "choose_payment"
        session["state"] = s
        msg = ("اختر طريقة الدفع: كاش أو أونلاين" if lang == "ar" else "Choose payment method: Cash or Online.")
        return make_chat_response(msg, lang)
