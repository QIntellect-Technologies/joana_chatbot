...
def whatsapp_webhook():
        # --- Detect WhatsApp clear chat event (empty message after previous order) ---
        # If user sends an empty message and there is a previous order, treat as new session
        if not (from_button or is_voice) and user_text.strip() == "" and ctx.get("state", {}).get("order"):
...
def process_whatsapp_message(
    ...
    # --- If previous order is complete (payment done), always start new session on next message ---
    prev_order = prev_state.get("order") if prev_state else []
    prev_stage = prev_state.get("stage") if prev_state else None
    prev_total = prev_state.get("total") if prev_state else 0
    # If previous order exists and stage is payment or after, reset session for new order
    if prev_order and prev_stage in ("payment", "choose_payment", "await_feedback", "order_complete"):
        session["state"] = {"stage": None, "order": [], "total": 0, "last_item": None, "last_qty": 0,
                            "last_confirmed_item": None, "pending_item": None, "spice_queue": [], "generic_queue": []}
        session["messages"] = []
        resp = chat()
        WHATSAPP_SESSIONS[user_number] = {
        ...
        # Send clear new order message
        reply = (
            "تم بدء طلب جديد! 🆕\n\nماذا تريد أن تطلب؟\n\n" + build_branches_message("ar")
            if (user_lang_hint or ctx.get("lang")) == "ar" else
            "New order started! 🆕\n\nWhat would you like to order?\n\n" + build_branches_message("en")
        )
        send_whatsapp_text(user_number, reply)
        return data
