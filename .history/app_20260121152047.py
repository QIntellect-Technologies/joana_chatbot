...
def handle_multi_item_text(msg_raw, s, MENU, lang):
    ...
    for spec in specific_items:
        ...
        # ✅ Sandwiches (Specific) -> Add directly
        elif cat == "sandwiches":
            s.setdefault("order", [])
            s["order"].append({
                "item": resolved,
    ...
    if food_only:
        ...
        s["generic_queue"].extend(list(food_only))
    ...
    if bs_only:
        s.setdefault("generic_queue", [])
        s["generic_queue"].extend(bs_only)
...
def _start_next_generic_from_queue(s: dict, MENU: dict, lang: str):
    ...
    if kind == "sandwich":
        s["stage"] = "await_specific_sandwich"
        s["sand_page"] = 0
        w_word = "wraps/sandwiches" if kind == "sandwich" else "sandwiches"
        ask_next = (
            f"لديك {qty} ساندويتش/راب. اختر النوع:"
            if lang == "ar"
            else f"You ordered {qty} sandwich(es)/wraps. Please choose which one:"
        )
        return ask_next
...
def detect_generic_requests_ordered(msg: str):
    ...
    def add(kind, pos):
        ...
        if kind == "sandwich":
            # treat wraps as generic sandwich
            if "wrap" in raw.lower() and not msg_has_specific_sandwich(raw):
                 # if detected via 'wrap' keyword, maybe we should unify logic
                 pass
            elif not msg_has_specific_sandwich(raw):
                 pass
            
            # Simple check: If specific sandwich exists, we skip generic.
            # But what if user says "1 Falafel Sandwich and 5 wraps"?
            # msg_has_specific_sandwich checks if ANY sandwich name is in msg.
            # "Falafel Sandwich" is in msg -> True.
            # So generic "wrap" gets skipped.
            # Ideally we want to allow generic wrap even if specific sandwich exists if they are distinct tokens.
            # But for now, let's keep it simple and just ensure "wrap" maps to "sandwich" kind.
            
            # IMPROVEMENT: Check specifically for generic keywords
            pass

        if kind == "sandwich":
            # Just ensure we map 'wrap' correctly in the caller regex/loop
            pass
    ...
    # sandwiches / WRAPS
    # -----------------------
    for k in ["sandwich", "sandwiches", "wrap", "wraps"]:
        p = t.find(k)
        if p != -1:
            add("sandwich", p)
            break
...
def parse_intelligent_order(msg: str, lang: str = "en") -> dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            "MENU BY CATEGORY:\n"
            f"{menu_context}\n\n"
            "TASK: Parse customer's natural language order (may have typos, mixed language).\n\n"
            "RULES:\n"
            "1. If user says generic category (\"burger\", \"sandwich\", \"wrap\", \"drink\", \"meal\", \"juice\") → type=\"generic\"\n"
            "2. If user mentions SPECIFIC item name (\"Beef Burger\", \"Coffee\", \"Pepsi\") → type=\"specific\"\n"
            "3. Handle typos intelligently: cofee→Coffee, burgur→Burger, sandwihc→Sandwich, wrapps→Wrap\n"
            "4. Extract quantities from text (\"2 burger 3 coffee\" → burger qty=2, coffee qty=3)\n"
            "5. Detect preferences: spicy/non-spicy for burgers\n\n"
            "OUTPUT (JSON ONLY):\n"
            "{\n"
            "  \"items\": [\n"
            "    {\"type\": \"generic\", \"category\": \"burger\", \"qty\": 2},\n"
            ...
            "  ]\n"
            "}\n\n"
            "- category must match menu categories: burgers_meals, sandwiches, drinks, meals, juices, snacks_sides\n"
            "- Note: 'wrap' is allowed as category for generic items, or map it to 'sandwich'.\n"
            "- name must be exact menu item name (English)\n"
        ...
        for it in items:
            ...
            if item_type == "generic":
                ...
                if category in ["burger", "burgers"]:
                    category = "burger"
                elif category in ["sandwich", "sandwiches", "wrap", "wraps"]:
                    category = "sandwich"
                elif category in ["meal", "meals"]:
                    category = "meals"
                elif category in ["juice", "juices"]:
                    category = "juices"
                elif category in ["drink", "drinks"]:
                    category = "drinks"
...
def chat():
    ...
    if s.get("stage") == "await_specific_sandwich" and not msg_l.startswith("pick_sandwich_"):
        if not is_cancel_text(msg_raw):
            ...
            if picked_key:
                # We found a specific sandwich.
                # We assume the user wants to fulfill the pending generic qty with this specific sandwich
                qty = int(s.get("last_qty") or 1)
                info = MENU.get(picked_key) or {}
                price = float(info.get("price") or 0)
                
                # Check if it's a Zinger variant (needs spice choice?)
                # Actually user data implies "Regular Tortilla Zinger" is a sandwich but might need spice? 
                # The data shows "Regular Tortilla Zinger" has price but no separate spicy option in name?
                # Wait, "Regular Tortilla Zinger" vs "Spicy Tortilla Zinger" are separate items in Excel.
                # So if user picked "Regular...", it is non-spicy by definition.
                # If user picked "Spicy...", it is spicy by definition.
                # So no need to ask spice. Just add.
                ...
                p_name = info.get("name_en") if lang != "ar" else info.get("name_ar")
                reply = (
                    f"{p_name} ×{qty} added.<br>Order summary:<br>" + "<br>".join(summary) + "<br><br>Anything else?"
                    if lang != "ar"
                    else f"تم إضافة {p_name} ×{qty}.<br>ملخص الطلب:<br>" + "<br>".join(summary) + "<br><br>أي شيء آخر؟"
                )
