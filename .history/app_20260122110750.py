...
def send_menu_image(to_number: str, lang: str = "en"):
    """Helper function to send the menu image with appropriate caption"""
    ...
    return send_whatsapp_image(to_number, image_url, caption)
...
def whatsapp_webhook():
    ...
    # Check if user is asking something completely off-topic (only for text messages, not buttons)
    if not from_button:
        irrelevant_result = check_if_irrelevant_question(user_text, lang)
        if irrelevant_result and irrelevant_result.get("is_irrelevant"):
            print(f"🚫 IRRELEVANT QUESTION DETECTED: {user_text}")
            polite_msg = irrelevant_result.get("response", "")
            
            # Send polite message
            if polite_msg:
                send_whatsapp_text(user_number, polite_msg)
            
            # Send menu image
            send_menu_image(user_number, lang)
            
            # Send category buttons
            if lang == "ar":
                category_buttons = [
                    {"id": "burgers", "title": "🍔 برجر"},
                    {"id": "sandwiches", "title": "🥪 ساندويتشات"},
