...
def check_if_irrelevant_question(msg: str, lang: str = "en") -> dict:
    """
    Use AI to detect if user's question is irrelevant to food ordering.
    ...
    # Use AI to detect irrelevant questions
    system_prompt = (
...
# CATEGORY UI SENDING
# =========================================================
def send_menu_image(user_number: str, caption: str = None):
    """Send the menu image to user"""
    image_url = "https://qintellecttechnologies.com/joana_chatbot/static/menu.PNG"
    if caption is None:
        # Auto-detect language from session
        ctx = WHATSAPP_SESSIONS.get(user_number, {})
        lang = ctx.get("lang", "en")
        caption = "هذه قائمتنا 📋" if lang == "ar" else "Here is our menu 📋"
...
def whatsapp_webhook():
    ...
    # ✅ INTELLIGENT IRRELEVANT QUESTION DETECTION (GLOBAL - WHATSAPP)
    # Detect and politely redirect ANY irrelevant questions at ANY stage
    if not from_button and user_text:  # Only check text messages, not button clicks
        irrelevant_check = check_if_irrelevant_question(user_text, lang)
        if irrelevant_check.get("is_irrelevant"):
            polite_response = irrelevant_check.get("polite_response")
            print(f"🚫 IRRELEVANT QUESTION DETECTED (WhatsApp): '{user_text}'")
            print(f"💬 Sweet Response: {polite_response}")
            
            # Send the sweet polite response
            if polite_response:
                send_whatsapp_text(user_number, polite_response)
                save_message(
                    customer_id=customer_id,
                    order_id=None,
            ...
            # Show menu to redirect them back to ordering
            time.sleep(0.5)  # Small delay between messages
            send_menu_image(user_number)
            time.sleep(0.3)
...
def chat():
    ...
    # ✅ INTELLIGENT IRRELEVANT QUESTION DETECTION (GLOBAL)
    # Detect and politely redirect ANY irrelevant questions at ANY stage
    if not from_button and msg_raw:  # Only check text messages, not button clicks
        irrelevant_check = check_if_irrelevant_question(msg_raw, lang)
        if irrelevant_check.get("is_irrelevant"):
            polite_response = irrelevant_check.get("polite_response")
            print(f"🚫 IRRELEVANT QUESTION DETECTED: '{msg_raw}'")
            print(f"💬 Sweet Response: {polite_response}")
    ...
    if not from_button and is_non_menu_item_request(msg_raw):
        print(f"⚠️ NON-MENU ITEM DETECTED (not on menu): '{msg_raw}'")
        
        # Build sweet polite rejection with menu redirect
        rejection_msg = (
            "عذراً عزيزي 😊، للأسف هذا الصنف غير متوفر في قائمتنا حالياً.\n\n"
            "لكن لدينا الكثير من الخيارات اللذيذة الأخرى! 🍔☕\n"
            "دعني أريك قائمتنا وستجد بالتأكيد شيئاً رائعاً! 👇"
            if lang == "ar" else
            "Sorry dear 😊, unfortunately that item is not available on our menu right now.\n\n"
            "But we have many other delicious options! 🍔☕\n"
            "Let me show you our menu and you'll definitely find something amazing! 👇"
        )
        
        # Send sweet message and show menu
        send_whatsapp_text(user_number, rejection_msg)
        save_message(
            customer_id=customer_id,
            order_id=None,
