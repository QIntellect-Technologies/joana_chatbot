...
def transcribe_audio_from_cloud(media_id: str) -> str:
    """
    Transcribe WhatsApp voice message using OpenAI Whisper API.
    Downloads audio from WhatsApp, converts to compatible format, and transcribes.
    """
    if not media_id:
        return ""
    ...
    try:
        ...
        # WhatsApp usually sends .ogg format, but Whisper supports many formats
        ...
        # Auto-detects language (English or Arabic)
        with open(temp_audio_path, "rb") as audio_file:
            transcript = client.audio.transcriptions.create(
                model="whisper-1",
                file=audio_file,
                response_format="text",
                language=None,  # Auto-detect (supports both English and Arabic)
            )
