...
def whatsapp_webhook():
    ...
    # 🚫 GLOBAL GUARD: Unsupported/irrelevant food items (e.g., pizza)
    msg_low_global = user_text.lower()
    unsupported_food_global = [
        "pizza", "piza", "pizaa", "pissa", "picca", "pitsa", "pitsa",
        "pasta", "biryani", "shawarma", "karahi", "sushi", "tacos", "burrito", "noodles",
        "بيتزا", "باستا", "رز", "شاورما", "مكرونة"
    ]
    if any(k in msg_low_global for k in unsupported_food_global):
        reply_guard = (
            "عذراً، يمكن طلب عناصر القائمة فقط. من فضلك اختر من القائمة."
            if lang == "ar" else
        ...
        # Keep state but do not proceed further
        return "ok", 200
    ...
    # 🚫 EARLY GUARD: Unsupported/abusive queries (e.g., pizza)
    ...
    unsupported_food = [
        "pizza", "picca", "pizaa", "piza", "pissa", "pitsa", "pitsa",
        "pasta", "biryani", "shawarma", "karahi", "sushi", "tacos", "burrito", "noodles",
        "بيتزا", "باستا", "رز", "شاورما", "مكرونة"
    ]
    is_abusive_guard = any(k in msg_low_guard for k in abusive_keywords)
    is_unsupported_food_guard = any(k in msg_low_guard for k in unsupported_food)

    if not from_button and (is_abusive_guard or is_unsupported_food_guard):
        reply_guard = (
...
def chat():
    ...
    # 🚫 Guard against irrelevant/unsupported/abusive queries
    ...
    unsupported_food = [
        "pizza", "pasta", "biryani", "shawarma", "karahi", "sushi", "tacos", "burrito", "noodles",
        "pizzaa", "piza", "piza", "بيتزا", "باستا", "رز", "شاورما", "مكرونة"
    ]
    is_abusive = any(k in msg_low for k in abusive_keywords)
    is_unsupported_food = any(k in msg_low for k in unsupported_food)

    if is_abusive or is_unsupported_food:
        reply = (
            "آسف، لا يمكنني المساعدة في ذلك. يمكن طلب عناصر القائمة فقط. الرجاء الاختيار من القائمة."
            if lang == "ar" else
            "Sorry, I can't assist with that. Only menu items can be ordered. Please choose from the menu."
