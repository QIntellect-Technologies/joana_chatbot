...
def handle_multi_item_text(msg_raw, s, MENU, lang):
    ...
    # ✅ STEP 1: AI-POWERED ARABIC TYPO CORRECTION (ONLY for Arabic messages)
    # This fixes common mistakes BEFORE parsing: فشار→بوفك دجاج, تورتيلا→تورتيا, etc.
    # English messages skip this entirely to avoid any performance impact
    corrected_msg = correct_arabic_typos_with_ai(msg_raw) if lang == "ar" else msg_raw
    
    # ✅ STEP 2: USE INTELLIGENT PARSER (with corrected input for Arabic, original for English)
    parsed = parse_intelligent_order(corrected_msg, lang)
    ...
    if not all_items:
        ...
        bs_generics = detect_generic_requests_ordered(corrected_msg)
        food_generics = detect_food_generic_requests_ordered(corrected_msg)
        ...
        # Check if any SPECIFIC item is already a parsed wrap/tortilla
        has_specific_wrap = False
        for item in all_items:
            if item.get("type") == "specific":
                name = (item.get("name") or "").lower()
                if "wrap" in name or "tortilla" in name or "تورتيلا" in name or "تورتيا" in name:
                    has_specific_wrap = True
                    break
        
        if has_wrap_word and not has_sandwich_in_generics and not has_specific_wrap:
...
def looks_like_multi_item_text(msg: str) -> bool:
    ...
    arabic_num_count = sum(1 for num in arabic_number_words if num in original)
    
    # ✅ Count Arabic dual form endings (ان، ين، تان، تين) - indicates quantity
    dual_form_matches = re.findall(r'\w+(ان|ين|تان|تين)\b', original)
    dual_count = len(dual_form_matches)
    ...
    if any(sep in t for sep in [" and ", " & ", " + ", " ثم ", " , "]) or has_arabic_separator:
        # BUT still require at least one qty (digit, Arabic number word, or dual form)
        has_qty = bool(re.search(r"\b\d+\b", t) or arabic_num_count > 0 or dual_count > 0)
        if has_qty:
            return True
    ...
    # ✅ Enhanced fallback: Multiple quantities (digits OR Arabic numbers OR dual forms)
    digit_nums = re.findall(r"\b\d+\b", t)
    total_qty_indicators = len(digit_nums) + arabic_num_count + dual_count
    
    if total_qty_indicators >= 2:
        return True
...
def parse_spice_split(msg: str, total_qty: int):
    ...
    # ✅ Detect number words (one..ten) and digits
    # First, find digits
    nums = [int(x) for x in re.findall(r"\b\d+\b", text)]
    
    # Then look for english number words if we don't have enough digits or just to be safe
    # Map words to ints
    word_to_num = {
    ...
    # Find words in order? Or just find all and append? 
    # Better to find ALL numbers (digits and words) in appearance order to respect "1 spicy, 2 non" vs "2 spicy 1 non"
    # Re-scan text for both patterns together
    
    all_matches = []
    # Tokenize by space to preserve order roughly, or use regex for both
    # Regex for both:
    pattern = r"\b(\d+|one|two|three|four|five|six|seven|eight|nine|ten)\b"
    for match in re.finditer(pattern, text):
        val_str = match.group(1).lower()
        if val_str.isdigit():
            all_matches.append(int(val_str))
        else:
            all_matches.append(word_to_num.get(val_str, 0))
            
    # Use the ordered list if it found anything, otherwise fallback to just digits
    if all_matches:
        nums = [n for n in all_matches if n > 0]
...
def resolve_menu_item(raw: str):
    """
    Resolve user input to menu item key with fuzzy matching.
    Supports: exact match, substring, partial word match, Arabic variants
    Returns: (menu_key, confidence_score)
    """
    if not raw:
        return None, 0.0

    t = normalize_arabic_variants(raw.lower().strip())
    t = re.sub(r"\s+", " ", t)
    ...
    # ✅ STEP 2: Check against both English AND Arabic names
    best_match = None
    best_score = 0.0
    
    for menu_key, info in MENU.items():
        name_en = (info.get("name_en") or menu_key).lower().strip()
        name_ar = (info.get("name_ar") or "").strip()
        
        # Normalize for comparison
        menu_key_norm = normalize_arabic_variants(menu_key.lower())
        name_en_norm = normalize_arabic_variants(name_en)
        name_ar_norm = normalize_arabic_variants(name_ar) if name_ar else ""
        ...
        if name_ar_norm and t == name_ar_norm:
            return menu_key, 1.0
        
        # ✅ CRITICAL: Partial word matching (e.g., "popcorn" → "chicken popcorn")
        # If user's input is a complete word found in the menu item name
        user_words = set(t.split())
        en_words = set(name_en_norm.split())
        ar_words = set(name_ar_norm.split()) if name_ar_norm else set()
        
        # Check if ALL user words are in menu item (partial match)
        if user_words and user_words.issubset(en_words | ar_words):
            # Score based on word coverage
            score = len(user_words) / max(len(en_words | ar_words), 1)
            if score > best_score:
                best_match = menu_key
                best_score = score
        
        # Substring match (lower priority)
        if menu_key_norm in t and len(menu_key_norm) > len(best_match or ""):
            best_match = menu_key
            best_score = 0.85
        
        # Arabic substring match
        if name_ar_norm and name_ar_norm in t and len(name_ar_norm) > len(best_match or ""):
            best_match = menu_key
            best_score = 0.85
        
        # English name substring in user input
        if name_en_norm in t and len(name_en_norm) > len(best_match or ""):
            best_match = menu_key
            best_score = 0.85
...
def correct_arabic_typos_with_ai(msg: str) -> str:
    """
    ✅ AI-POWERED ARABIC TYPO CORRECTION
    Uses GPT to intelligently fix Arabic spelling mistakes and map to menu items.
    This makes the system extremely tolerant of Arabic typos!
    
    Examples:
    - "تورتيلا" → "تورتيا" (menu spelling)
    - "فشار" → "بوفك دجاج" (popcorn → chicken popcorn)
    - "شكشوكه" → "شكشوكة" (typo fix)
    - "كبابب دجاج" → "كباب دجاج" (extra letter)
    """
    if not msg or not client:
        return msg
    ...
    # Build menu context for Arabic correction
    arabic_menu_items = []
    for key, info in MENU.items():
        name_ar = (info.get("name_ar") or "").strip()
        name_en = (info.get("name_en") or key).strip()
        if name_ar:
            arabic_menu_items.append(f"{name_ar} ({name_en})")
    
    menu_context = "\n".join(arabic_menu_items[:40])  # First 40 items
    
    system_prompt = (
        "You are a spelling correction expert for Arabic food orders. Your ONLY job is to fix spelling mistakes.\n\n"
        "CRITICAL RULES - MUST FOLLOW:\n"
        "1. ONLY fix obvious typos and spelling errors\n"
        "2. NEVER add explanations, comments, or new words\n"
        "3. NEVER say 'there is no X' or 'not available' - just fix spelling\n"
        "4. Keep numbers exactly as written (٦، ٨، ١، 2، 3، 5)\n"
        "5. Keep sentence structure and all words from input\n"
        "6. If word matches menu item closely, use correct menu spelling\n"
        "7. Return ONLY the corrected text, nothing else\n\n"
        "Menu items for reference (Arabic - English):\n"
        f"{menu_context}\n\n"
        "Common corrections:\n"
        "- 'تورتيلا' → 'تورتيا' (menu spelling)\n"
        "- 'شكشوكه' → 'شكشوكة' (typo fix)\n"
        "- 'كبابب' → 'كباب' (extra letter)\n"
        "- 'برغر' → 'برجر' (spelling variant)\n"
        "- 'بطاطا' → 'بطاطس' (correct spelling)\n"
        "- 'ساندوتش' → 'ساندويتش' (spelling variant)\n"
        "- 'قهوه' → 'قهوة' (typo fix)\n"
        "- 'فشار' → 'بوفك دجاج' (popcorn → chicken popcorn)\n\n"
        "EXAMPLES:\n"
        "Input: 'أريد قهوتين'\n"
        "Output: 'أريد قهوتين'\n\n"
        "Input: '٦ فشار، ٨ بطاطس مقليه'\n"
        "Output: '٦ بوفك دجاج، ٨ بطاطس مقلية'\n\n"
        "Input: 'كبابب دجاج جامبووو'\n"
        "Output: 'كباب دجاج جامبو'\n\n"
        "NEVER OUTPUT:\n"
        "❌ 'لا توجد قهوة في قائمة الطعام' (NO explanations!)\n"
        "❌ 'Sorry, ...' (NO apologies!)\n"
        "❌ 'This item is not available' (NO availability comments!)\n\n"
        "Return ONLY the corrected Arabic text from the user's input."
    )
    ...
    try:
        ...
        # ✅ STRONG Safety checks - reject invalid AI outputs
        # 1. Check for "لا توجد" (there is no), "غير متوفر" (not available), "Sorry", etc.
        invalid_phrases = [
            "لا توجد", "لا يوجد", "غير متوفر", "غير موجود", "not available", 
            "sorry", "عذراً", "آسف", "لا أستطيع", "cannot", "can't"
        ]
        has_invalid = any(phrase in corrected.lower() for phrase in invalid_phrases)
        
        # 2. Check length is reasonable (not too different from original)
        length_ok = len(corrected) >= len(msg) * 0.5 and len(corrected) <= len(msg) * 2.5
        
        # 3. Check it's not empty and has Arabic content
        has_content = bool(corrected and any('\u0600' <= ch <= '\u06ff' for ch in corrected))
        
        # 4. Use correction ONLY if all checks pass
        if has_content and length_ok and not has_invalid:
            print(f"✅ Arabic typo corrected: '{msg}' → '{corrected}'")
            return corrected
        else:
            reason = "empty" if not has_content else "invalid phrase" if has_invalid else "unreasonable length"
            print(f"⚠️ Arabic correction rejected ({reason}): '{corrected}' | Using original: '{msg}'")
            return msg
            
    except Exception as e:
        print(f"❌ Arabic correction error: {repr(e)}")
        return msg  # Fail gracefully, return original
...
def parse_intelligent_order(msg: str, lang: str = "en") -> dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            "⚠️ EXCEPTION: If the user names a SPECIFIC wrap (e.g. 'Tortilla Chicken'), extract it as 'specific' item.\n"
            "   DO NOT extract a generic 'sandwich' in addition to the specific wrap unless the user said 'and a sandwich'.\n"
            "   Example: '1 Tortilla Chicken' → 1 Specific Item (NO generic sandwich).\n"
            "   Example: '2 wraps' → 2 Generic Sandwich.\n"
            "\n"
            "RULE 2 - SIDES/SNACKS:\n"
            ...
            "⚠️ SMART MAPPING (use EXACT menu names):\n"
            "  - 'فشار' OR 'popcorn' OR 'بوفك' → \"Chicken Popcorn\" (NOT just \"Popcorn\")\n"
            "  - 'تورتيلا دجاج' OR 'tortilla chicken' → \"Tortilla Chicken Jambo\"\n"
            "  - 'شكشوكة' OR 'shashukah' → \"Sandwiches Shashukah\"\n"
            "  - 'كباب دجاج' OR 'kabab chicken' → \"Kabab Chicken Jambo\"\n"
            ...
            "\n"
            "EXAMPLE 7 (Popcorn mapping):\n"
            "Input: \"٦ فشار و ٨ بطاطس مقلية\"\n"
            "Output:\n"
            "{\n"
            "  \"items\": [\n"
            "    {\"type\": \"specific\", \"name\": \"Chicken Popcorn\", \"qty\": 6, \"category\": \"snacks_sides\"},\n"
            "    {\"type\": \"specific\", \"name\": \"French Fries\", \"qty\": 8, \"category\": \"snacks_sides\"}\n"
            "  ]\n"
            "}\n"
            "✅ Correct: 'فشار' (popcorn) → \"Chicken Popcorn\" (actual menu item)\n"
            "\n"
            "EXAMPLE 8 (Typos and informal):\n"
            ...
            "\n"
            "EXAMPLE 11 (Specific Sides):\n"
            "Input: \"One sweet potato and one sweet corn\"\n"
            "Output:\n"
            "{\n"
            "  \"items\": [\n"
            "    {\"type\": \"specific\", \"name\": \"Sweet Potato\", \"qty\": 1, \"category\": \"snacks_sides\"},\n"
            "    {\"type\": \"specific\", \"name\": \"Sweet Corn\", \"qty\": 1, \"category\": \"snacks_sides\"}\n"
            "  ]\n"
            "}\n"
            "✅ Correct: Specific sides matched exactly\n"
            "\n"
            "EXAMPLE 12 (Specific Wraps - NO GENERIC):\n"
            "Input: \"One spicy tortilla and one regular\"\n"
            "Output:\n"
            "{\n"
            "  \"items\": [\n"
            "    {\"type\": \"specific\", \"name\": \"Tortilla Chicken Jambo\", \"qty\": 1, \"category\": \"sandwiches\", \"spicy\": \"spicy\"},\n"
            "    {\"type\": \"specific\", \"name\": \"Tortilla Chicken Jambo\", \"qty\": 1, \"category\": \"sandwiches\", \"spicy\": \"non-spicy\"}\n"
            "  ]\n"
            "}\n"
            "✅ Correct: Specific items only. NO generic 'sandwich' item added.\n"
            "\n"
            "EXAMPLE 11 (Arabic):\n"
        ...
        for it in items:
            ...
            if item_type == "generic":
                ...
                # ✅ SMART NAME MAPPING: Handle common user terms → actual menu items
                # This helps when LLM extracts generic terms that need to map to specific menu items
                name_lower = name.lower()
                
                # Popcorn variants → Chicken Popcorn
                if name_lower in ["popcorn", "pop corn", "فشار", "بوفك"]:
                    name = "Chicken Popcorn"
                
                # Tortilla variants → look for closest match
                if "tortilla" in name_lower or "تورتيلا" in name_lower or "تورتيا" in name_lower:
                    # If it mentions chicken/دجاج → Tortilla Chicken
                    if "chicken" in name_lower or "دجاج" in name_lower:
                        if "jambo" in name_lower or "جامبو" in name_lower:
                            name = "Tortilla Chicken Jambo"
                        else:
                            name = "Tortilla Chicken Jambo"  # Default to Jambo
                    # If it mentions zinger/زنجر
                    elif "zinger" in name_lower or "زنجر" in name_lower:
                        if "spicy" in name_lower or "حار" in name_lower:
                            name = "Spicy Tortilla Zinger"
                        else:
                            name = "Regular Tortilla Zinger"
                
                # Shashukah variants
                if "shashukah" in name_lower or "شكشوكة" in name_lower or "shakshuka" in name_lower:
                    name = "Sandwiches Shashukah"
                
                # Kabab variants
                if ("kabab" in name_lower or "كباب" in name_lower) and ("chicken" in name_lower or "دجاج" in name_lower):
                    if "jambo" in name_lower or "جامبو" in name_lower:
                        name = "Kabab Chicken Jambo"
                    else:
                        name = "Kabab Chicken Jambo"  # Default to Jambo
...
def transcribe_audio_from_cloud(media_id: str, priority_lang=None) -> str:
    """
    ✅ OPENAI WHISPER TRANSCRIPTION for WhatsApp Voice Messages
    
    Features:
    - Auto-detects language (English/Arabic)
    - Supports all audio formats (.ogg, .mp3, .m4a, .wav)
    - Handles numbers correctly (converts words to digits)
    - Robust error handling with detailed logging
    
    Cost: ~$0.006 per minute of audio (very affordable!)
    """
    if not media_id:
        print("⚠️ No media_id provided to transcribe_audio_from_cloud")
        return "[ERROR: No media_id provided to backend]"
    if not WHATSAPP_TOKEN:
        print("⚠️ WHATSAPP_TOKEN not configured in environment")
        return "[ERROR: WhatsApp token not configured on server]"
    if not client:  # Need OpenAI client
        print("❌ OpenAI client not available for transcription (OpenAI API key missing or invalid)")
    ...
    try:
        print(f"🎤 Starting voice transcription for media_id: {media_id}")
        # Step 1: Get media URL from WhatsApp
        meta_resp = requests.get(meta_url, headers=headers, timeout=30)
        try:
            meta_resp.raise_for_status()
        except Exception as e:
            print(f"❌ WhatsApp media metadata fetch failed: {repr(e)}")
            print(f"   Response: {getattr(meta_resp, 'text', '')[:200]}")
            return f"[ERROR: WhatsApp media metadata fetch failed: {repr(e)}]"
        meta = meta_resp.json()
        file_url = meta.get("url")
        if not file_url:
            print("❌ No file URL from WhatsApp media metadata")
            return "[ERROR: No file URL from WhatsApp media metadata]"

        # Step 2: Download audio file
        print(f"📥 Downloading audio from WhatsApp...")
        file_resp = requests.get(file_url, headers=headers, timeout=60)
        try:
            file_resp.raise_for_status()
        except Exception as e:
            print(f"❌ WhatsApp audio download failed: {repr(e)}")
            print(f"   Response: {getattr(file_resp, 'text', '')[:200]}")
            return f"[ERROR: WhatsApp audio download failed: {repr(e)}]"
        audio_bytes = file_resp.content
        if len(audio_bytes) < 100:  # Too small to be valid audio
            print(f"❌ Audio file too small ({len(audio_bytes)} bytes), likely corrupted")
            return f"[ERROR: Audio file too small ({len(audio_bytes)} bytes)]"
        print(f"✅ Downloaded {len(audio_bytes)} bytes ({len(audio_bytes)/1024:.1f} KB) of audio")

        # Step 3: Save to temporary file (OpenAI Whisper API needs file object)
        ...
        try:
            with open(temp_audio_path, "rb") as audio_file:
                transcript = client.audio.transcriptions.create(
                    ...
                    prompt=(
                        "Transcribe in English." if priority_lang == "en" else
                        "Transcribe in Arabic." if priority_lang == "ar" else
                        "Transcribe in English or Arabic only. If unclear, output nothing."
                    ),
                    temperature=0.2,
                )
        except Exception as e:
            print(f"❌ Whisper API call failed: {repr(e)}")
            import traceback
            traceback.print_exc()
            return f"[ERROR: Whisper API call failed: {repr(e)}]"
        transcribed_text = (transcript or "").strip()
        
        # ✅ GUARD: Check for unsupported scripts (e.g., Hindi, Cyrillic, Chinese)
        # Allowed: English (A-Za-z), Arabic (\u0600-\u06FF), Digits, Common Punctuation
        if transcribed_text:
            # Remove allowed characters
            cleaned = re.sub(r"[a-zA-Z0-9\u0600-\u06FF\s.,!?;:'\"-]", "", transcribed_text)
            # If significant "foreign" characters remain (more than 20% of length), reject
            if len(cleaned) > len(transcribed_text) * 0.2 and len(transcribed_text) > 5:
                print(f"⚠️ REJECTED TRANSCRIPTION (Unsupported Language): {transcribed_text}")
                return "[ERROR: Unsupported language detected. Please speak English or Arabic.]"
                
            # ✅ GUARD: Check for hallucinated repetitions (e.g. "and and and and")
            if len(transcribed_text) > 30:
                words = transcribed_text.split()
                if len(words) > 10:
                    # check if >50% of words are identical
                    most_common = max(set(words), key=words.count)
                    if words.count(most_common) > len(words) * 0.5:
                        print(f"⚠️ REJECTED TRANSCRIPTION (Repetitive Hallucination): {transcribed_text}")
                        return "[ERROR: Voice unclear]"

            has_arabic = any('\u0600' <= ch <= '\u06ff' for ch in transcribed_text)
            detected_lang = "Arabic" if has_arabic else "English"
            print(f"✅ Whisper transcription ({detected_lang}): '{transcribed_text}'")
            print(f"   Length: {len(transcribed_text)} chars, Words: {len(transcribed_text.split())}")
            return transcribed_text
        else:
            print("⚠️ Whisper returned empty transcription")
            return "[ERROR: Whisper returned empty transcription]"
    except requests.exceptions.Timeout as e:
        print(f"❌ Timeout downloading/transcribing voice: {repr(e)}")
        return f"[ERROR: Timeout downloading/transcribing voice: {repr(e)}]"
    except requests.exceptions.HTTPError as e:
        print(f"❌ HTTP error during voice processing: {repr(e)}")
        if hasattr(e.response, 'text'):
            print(f"   Response: {e.response.text[:200]}")
        return f"[ERROR: HTTP error during voice processing: {repr(e)}]"
    except Exception as e:
        print(f"❌ Voice transcription error: {repr(e)}")
        import traceback
        traceback.print_exc()
        return f"[ERROR: Voice transcription error: {repr(e)}]"
    finally:
        if temp_audio_path:
            try:
...
def whatsapp_webhook():
    ...
    if msg_type == "interactive":
        ...
        text_from_voice = transcribe_audio_from_cloud(media_id, priority_lang=stored_lang)
        if not text_from_voice or text_from_voice.startswith("[ERROR:"):
            error_detail = text_from_voice if text_from_voice else "Unknown error"
            print(f"❌ Voice transcription failed: {error_detail}")
            lang_for_fallback = stored_lang if stored_lang in ("en", "ar") else "en"
            fallback_msg = (
                f"عذراً، لم أستطع فهم الرسالة الصوتية. اكتب طلبك نصاً\n\n[تفاصيل الخطأ: {error_detail}]"
                if lang_for_fallback == "ar"
                else f"Sorry, I couldn’t understand that voice message. Please type your order.\n\n[Error details: {error_detail}]"
            )
    ...
    # ✅ AI-POWERED ARABIC TYPO CORRECTION (WhatsApp)
    # Apply intelligent correction AFTER language detection, BEFORE processing
    if lang == "ar" and user_text and not from_button:
        user_text = correct_arabic_typos_with_ai(user_text)
    ...
    # ✅ ECHO TRANSCRIPTION TO USER (Requested Feature)
    if is_voice:
        echo_msg = f"🎤 You said: {user_text}" if lang != "ar" else f"🎤 قلت: {user_text}"
        send_whatsapp_text(user_number, echo_msg)
    ...
    def looks_like_order_or_menu(text: str) -> bool:
        ...
        # ✅ Check for Arabic order intent words (I want, give me, etc.)
        arabic_order_intents = [
            "أريد", "اريد", "عايز", "عاوز", "اعطني", "أعطني", 
            "ابغى", "ابي", "بدي", "بغيت", "اطلب", "أطلب"
        ]
        has_arabic_intent = any(intent in text for intent in arabic_order_intents)
        
        # ✅ Check for Arabic number words (واحد، اثنين، ثلاثة، etc.)
        arabic_number_words = [
            "واحد", "اثنين", "اثنتين", "ثلاثة", "ثلاث", "أربعة", "أربع",
            "خمسة", "خمس", "ستة", "ست", "سبعة", "سبع", "ثمانية", "ثمان",
            "تسعة", "تسع", "عشرة", "عشر"
        ]
        has_arabic_number = any(num in text for num in arabic_number_words)
        
        # ✅ Check for Arabic dual form endings (ان، ين، تان، تين) - indicates quantity
        has_dual_form = bool(re.search(r'\w+(ان|ين|تان|تين)\b', text))
        ...
        food_keywords = [
            "burger", "sandwich", "coffee", "drink", "meal", "wrap", "side", "fries",
            "pepsi", "water", "juice", "chicken", "beef", "snack",
            "برجر", "ساندويتش", "قهوة", "مشروب", "وجبة", "بطاطس", "دجاج", "كوب", "أكواب"
        ]
        has_food_keyword = any(keyword in text_lower for keyword in food_keywords)
        
        # ✅ Enhanced detection logic:
        # - Menu keywords → always order/menu request
        # - Arabic intent words → always order
        # - (Numbers OR Arabic number words OR dual forms) + food keywords → order
        # - Arabic number words alone (likely ordering) → order
        return (has_menu_keyword or 
                has_arabic_intent or 
                ((has_numbers or has_arabic_number or has_dual_form) and has_food_keyword) or
                (has_arabic_number and len(text.split()) <= 10))  # Short Arabic number phrases likely orders
    ...
    # ✅ TEXT-BASED CATEGORY SELECTION (when user types the category name instead of clicking button)
    # This handles cases like "Snacks & Sides", "burgers", "sandwiches", "drinks" typed as text
    category_text_map = {
        # Burgers/Meals
        "burgers": "burgers_meals", "burger": "burgers_meals", "meals": "burgers_meals",
        "برجر": "burgers_meals", "برجرات": "burgers_meals", "وجبات": "burgers_meals",
        "burgers meals": "burgers_meals", "burgers & meals": "burgers_meals",
        # Sandwiches
        "sandwiches": "sandwiches", "sandwich": "sandwiches", "سندويتشات": "sandwiches",
        "سندويش": "sandwiches", "سندوتش": "sandwiches", "سندويتش": "sandwiches",
        # Snacks & Sides
        "snacks": "snacks_sides", "snack": "snacks_sides", "sides": "snacks_sides",
        "side": "snacks_sides", "snacks & sides": "snacks_sides", "snacks and sides": "snacks_sides",
        "snacks sides": "snacks_sides", "snacks&sides": "snacks_sides",
        "مقبلات": "snacks_sides", "وجبات خفيفة": "snacks_sides", "سناك": "snacks_sides",
        # Drinks
        "drinks": "drinks", "drink": "drinks", "beverages": "drinks", "beverage": "drinks",
        "مشروبات": "drinks", "مشروب": "drinks",
        # Meals
        "meal": "meals", "وجبة": "meals",
        # Juices
        "juices": "juices", "juice": "juices", "عصائر": "juices", "عصير": "juices",
    }
    
    if clean in category_text_map:
        selected_cat = category_text_map[clean]
        WA_CATEGORY_STATE[user_number] = {"category": selected_cat, "index": 0}
        send_items_for_category(user_number, selected_cat, lang)
        return "ok", 200
    ...
    try:
        ...
        # ✅ SAFETY: Check if user_number exists before using it
        if 'user_number' in locals() and user_number:
            error_lang = locals().get('lang', 'en')
            send_whatsapp_text(user_number, "Sorry, something went wrong." if error_lang == "en" else "عذراً، حدث خطأ ما.")
        return "ok", 200
...
def chat():
    ...
    # ✅ STEP 0: AI-POWERED ARABIC TYPO CORRECTION (before normalization)
    # This intelligently fixes Arabic spelling mistakes using GPT
    if lang == "ar":
        msg_raw = correct_arabic_typos_with_ai(msg_raw)
    ...
    if s.get("stage") == "await_specific_burger" and not msg_l.startswith("pick_burger_"):
        # ✅ PRIORITIZE GLOBAL COMMANDS (Cancel / Multi-order voice)
        # Verify if user wants to cancel or switch context (e.g. "cancel", "new order", "2 burgers")
        if is_cancel_text(msg_raw) or looks_like_multi_item_text(msg_raw):
            print(f"🔍 DEBUG [await_specific_burger] ESCAPE HATCH: msg='{msg_raw}'")
            if is_cancel_text(msg_raw):
                # Let the main loop handle cancel (it will fall through to cancel check below?)
                # Actually, standard flow has specific cancel handlers? No, usually cancel handled via buttons/text.
                # But here we are inside the 'stage' block. We must return or break.
                # Best way: Check cancel -> if yes, let the standard cancel logic handle it (which is NOT shown in this snippet but usually global).
                # Wait, global cancel check is NOT visible in snippet provided.
                # Let's simple return 'handle_multi_item_text' if multi-item, or let cancel pass?
                pass 
            
            # If multi-item voice order
            if looks_like_multi_item_text(msg_raw) and not is_cancel_text(msg_raw):
                s["stage"] = None # Reset specific stage
                session["state"] = s
                return handle_multi_item_text(msg_raw, s, MENU, lang)

        # Let 'cancel' fall through (caught by is_cancel_text check inside loops or global handlers?)
    ...
    if s.get("stage") == "await_spice" and s.get("last_item"):
        ...
        # ✅ PRIORITY: HANDLE CANCEL IN SPICE STAGE
        if is_cancel_text(msg):
             # Let the Global Cancel Handler (lines 6302+) pick this up?
             # NO, because we are inside the 'stage' check block which might be exclusive?
             # Actually, the global cancel block is at `if s.get("order") and is_cancel_text(msg_raw)...` 
             # Is `await_spice` in the list of cancel-allowed stages there?
             # Let's check... line 6303 says: `s.get("stage") in ("payment", "choose_payment", "add_more", "cancel_choice", "cancel_item_select")`
             # It does NOT include "await_spice". So we MUST handle it here or add it there.
             # Handling it here is safer for now.
             
             s["stage"] = None 
             s["last_item"] = None
             s["last_qty"] = 0
             session["state"] = s
             # Recurse or return cancel message directly? 
             # Easiest: return simple cancel message for this item interaction
             return make_chat_response("Cancelled item selection." if lang!="ar" else "تم إلغاء اختيار الصنف.", lang)
    ...
    # This catches cases like "أريد قهوتين" (I want 2 coffees) that don't match simple patterns
    # English already works because find_menu_item() is more lenient with patterns
    # Run this for ALL stages where users might be adding items (None, add_more, await_specific_burger, await_specific_sandwich)
    # This ensures Arabic gets the same smart parsing as English at ALL stages
    if lang == "ar" and not is_cancel_text(msg) and s.get("stage") not in ("await_spice", "payment", "choose_payment", "await_feedback", "await_feedback_comment"):
        # Try to parse as an order using the intelligent parser
        try:
            parsed = parse_intelligent_order(msg, lang)
            items = parsed.get("items") or []
            
            # If LLM found menu items, process them using handle_multi_item_text logic
            if items:
                print(f"🔍 Arabic fallback parser found {len(items)} items in '{msg[:50]}...'")
                # Use same logic as handle_multi_item_text but inline to avoid recursion
                specific_items = [it for it in items if it.get("type") == "specific"]
                generic_items = [it for it in items if it.get("type") == "generic"]
                ...
                for spec in specific_items:
                    ...
                    # Burgers with spicy info → add directly
                    elif cat == "burgers_meals" and spicy_info in ("spicy", "non-spicy", "nonspicy"):
                        s.setdefault("order", [])
                        is_spicy = 1 if spicy_info == "spicy" else 0
                        is_nonspicy = 1 if spicy_info in ("non-spicy", "nonspicy") else 0
                        s["order"].append({
                        ...
                        info = MENU.get(resolved, {})
                        display_name = info.get("name_ar") if lang == "ar" else (info.get("name_en") or resolved).title()
                        spicy_label = " (حار)" if is_spicy else " (بدون حار)" if is_nonspicy else ""
                        added_lines.append(f"{qty} {display_name}{spicy_label}")
                    
                    # Burgers without spicy → queue for asking
                    elif cat == "burgers_meals":
                        s.setdefault("spice_queue", [])
                        s["spice_queue"].append({"item": resolved, "qty": qty})
                ...
                # Handle generics
                generics_list = []
                for gen in generic_items:
                    generics_list.append({"kind": gen.get("category"), "qty": gen.get("qty", 1)})
                
                # If we added items or have generics/spice queue, show success
                if added_lines or generics_list or s.get("spice_queue"):
                    # Process generics first
                    if generics_list:
                        bs_only = [g for g in generics_list if g.get("kind") in ("burger", "sandwich")]
                        food_only = [g for g in generics_list if g.get("kind") in ("meals", "juices", "drinks", "snacks_sides")]
                        
                        s.setdefault("generic_queue", [])
                        s["generic_queue"].extend(bs_only + food_only)
                        
                        prompt = _start_next_generic_from_queue(s, MENU, lang)
                        if prompt:
                            session["state"] = s
                            prefix = ("تمت إضافة: " + "، ".join(added_lines) + "<br><br>") if added_lines else ""
                            return make_chat_response(prefix + prompt, lang)
                    ...
                    if s.get("spice_queue"):
                        ...
                        session["state"] = s
                        prefix = ("تمت إضافة: " + "، ".join(added_lines) + "<br><br>") if added_lines else ""
                        ask_spice = f"بالنسبة لـ {nxt['qty']} {nxt['item']}، هل تفضلها حارة أم بدون حار؟"
                        return make_chat_response(prefix + ask_spice, lang)
                    
                    # Otherwise show updated summary
                    summary, total = build_order_summary_and_total(s["order"], lang)
                    s["total"] = total
                    s["stage"] = "add_more"
                    session["state"] = s
                    ...
                    reply = (
                        "تمت إضافة: " + "، ".join(added_lines) + "<br><br>" +
                        "✅ ملخص الطلب:<br>" + "<br>".join(summary) + 
                        f"<br><br><b>الإجمالي: {total:.2f} {CURRENCY_AR}</b><br><br>هل ترغب في إضافة شيء آخر؟"
                    )
                    return make_chat_response(reply, lang)
        except Exception as e:
            print(f"⚠️ Arabic fallback parser error: {e}")
            # Continue to conversational fallback

    # Smart LLM with context (only reached if not Arabic add_more, or parsing failed)
