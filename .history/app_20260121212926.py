...
FEEDBACK_DELAY_MINUTES = 1  # Feedback sent 1 minute after payment
...
def looks_like_multi_item_text(msg: str) -> bool:
    ...
    # ✅ NEW: Single item with quantity and food keyword (e.g., "add 7 meals", "10 burgers")
    # This allows escape from await_specific stages to process new orders
    if len(nums) >= 1:
        food_keywords = [
            "burger", "sandwich", "coffee", "drink", "meal", "wrap", "side", "fries",
            "pepsi", "water", "juice", "chicken", "beef", "snack", "tortilla",
            "برجر", "ساندويتش", "قهوة", "مشروب", "وجبة", "بطاطس", "دجاج", "عصير"
        ]
        if any(keyword in t for keyword in food_keywords):
            return True
...
def parse_spice_split(msg: str, total_qty: int):
    ...
    if has_half and has_spicy and has_non:
        ...
        return half_qty + remainder, half_qty
...
def parse_intelligent_order(msg: str, lang: str = "en") -> dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            "  - burger/burgers/برجر → category=\"burger\"\n"
            "  - sandwich/ساندويتش/wrap/wraps → category=\"sandwich\"\n"
...
def whatsapp_webhook():
    ...
    # 🧠 INTELLIGENT ORDER DETECTION - Check if message looks like an order
    def looks_like_order(text: str) -> bool:
        """Detect if text looks like a food order (has numbers + food keywords)"""
        if not text:
            return False
        ...
        # If has both numbers AND food keywords, it's likely an order
        return has_numbers and has_food_keyword
    
    # 🧠 INTELLIGENT QUESTION DETECTION - Check if message is a question/inquiry
    def looks_like_question_or_inquiry(text: str) -> bool:
        """Detect if text is a question about menu, prices, or service"""
        if not text:
            return False
        ...
        inquiry_keywords = [
            "menu", "show", "what", "which", "do you have", "have you", "tell me",
            "price", "cost", "how much", "available", "options", "choices",
            "قائمة", "اعرض", "ماذا", "ما", "هل عندك", "عندكم", "أسعار", "كم", "متوفر"
        ]
    ...
    # BUT skip welcome if it looks like an order OR a question/inquiry!
    if not from_button and (is_first_interaction or is_wa_greeting(user_text)):
        # If it looks like an order OR a question, DON'T show welcome - let normal processing handle it
        if is_first_interaction and (looks_like_order(user_text) or looks_like_question_or_inquiry(user_text)):
            # Initialize session but don't show welcome - let order processing handle it
            WHATSAPP_SESSIONS[user_number] = {
                "state": {"stage": None, "order": [], "total": 0, "last_item": None, "last_qty": 0, 
                         "last_confirmed_item": None, "pending_item": None, "spice_queue": [], "generic_queue": []},
                "messages": [],
            ...
            # Don't return - continue to order processing below
        else:
...
def chat():
    ...
    if s.get("stage") == "await_specific_burger" and not msg_l.startswith("pick_burger_"):
        ...
        if not is_cancel_text(msg_raw):
            # ✅ CATEGORY SWITCH: If user asks about different category, show it
            if "?" in msg_raw or any(word in msg_l for word in ["menu", "show", "what", "which", "available"]):
                category_switch_keywords = {
                    "sandwich": ("sandwiches", "Sandwiches"),
                    "wrap": ("sandwiches", "Wraps"),
                    "tortilla": ("sandwiches", "Wraps"),
                    "meal": ("meals", "Meals"),
                    "juice": ("juices", "Juices"),
                    "drink": ("drinks", "Drinks"),
                    "snack": ("snacks_sides", "Snacks & Sides"),
                }
                
                for keyword, (cat_key, cat_name) in category_switch_keywords.items():
                    if keyword in msg_l:
                        body, chunk, total = _format_category_page(MENU, cat_key, lang, page=0)
                        if body:
                            s["stage"] = None  # Reset stage
                            session["state"] = s
                            return make_chat_response(f"{cat_name}:\n\n{body}", lang)
            ...
            if "menu" not in msg_l:
                ask = "Please choose a valid burger." if lang != "ar" else "من فضلك اختر برجر صحيح."
                return make_chat_response(ask, lang)
    ...
    if s.get("stage") == "await_specific_sandwich" and not msg_l.startswith("pick_sandwich_"):
        if not is_cancel_text(msg_raw):
            ...
            if "menu" not in msg_l:
                ask = "Please choose a valid sandwich." if lang != "ar" else "من فضلك اختر ساندوتش صحيح."
                return make_chat_response(ask, lang)
    ...
    if "menu" in msg_l or "send menu" in msg_l or "price list" in msg_l:
        ...
        # SMART CATEGORY DETECTION - Check if asking about specific category
        category_keywords = {
            "burger": ("burgers_meals", "Burgers"),
            "sandwich": ("sandwiches", "Sandwiches"),
            "wrap": ("sandwiches", "Wraps"),
            "meal": ("meals", "Meals"),
            "juice": ("juices", "Juices"),
            "drink": ("drinks", "Drinks"),
            "snack": ("snacks_sides", "Snacks & Sides"),
            "side": ("snacks_sides", "Sides"),
            "fries": ("snacks_sides", "Fries"),
        }
        
        for keyword, (cat_key, cat_name) in category_keywords.items():
            if keyword in msg_l:
                body, chunk, total = _format_category_page(MENU, cat_key, lang, page=0)
                if body:
                    return make_chat_response(f"{cat_name}:\n\n{body}", lang)
    
    # ✅ SMART SINGLE-WORD CATEGORY INQUIRY - "meals?", "burger?", "wraps?"
    # If user asks a single word with question mark about a category, show that category
    if not intent and ("?" in msg_raw or len(msg_l.split()) <= 2):
        category_keywords = {
            "burger": ("burgers_meals", "Burgers"),
            "sandwich": ("sandwiches", "Sandwiches"),
            "wrap": ("sandwiches", "Wraps"),
    ...
    if s.get("stage") == "confirm_large_order":
        ...
        if any(confirm in msg_clean for confirm in ["yes", "y", "نعم", "تأكيد", "موافق", "ok"]):
            ...
            if pending_queue:
                ...
                for spec in remaining_specific:
                    ...
                    if qty >= 100:
                        ...
                        confirm_msg = (
                            f"⚠️ تريد طلب *{qty} {display_name2}*\n\n"
                            f"هذه كمية كبيرة جداً! هل أنت متأكد؟\n\n"
                            f"💰 الإجمالي: {qty * float(price):.2f} SAR\n\n"
                            f"اكتب 'نعم' للتأكيد أو 'لا' للإلغاء"
                            if lang == "ar" else
                            f"⚠️ You want to order *{qty} {display_name2}*\n\n"
                            f"This is a very large quantity! Are you sure?\n\n"
                            f"💰 Total: {qty * float(price):.2f} SAR\n\n"
                            f"Type 'yes' to confirm or 'no' to cancel"
                        )
            ...
            # ✅ CRITICAL: Check if there are items in generic_queue BEFORE showing summary
            # If generics exist, start processing them instead of showing summary
            if s.get("generic_queue"):
                prompt = _start_next_generic_from_queue(s, MENU, lang)
                if prompt:
                    session["state"] = s
                    reply = (
                        f"✅ تمت إضافة {pending['qty']} {display_name}<br><br>{prompt}"
                        if lang == "ar" else
                        f"✅ Added {pending['qty']} {display_name}<br><br>{prompt}"
                    )
                    return make_chat_response(reply, lang)
    ...
    if s.get("order") and is_cancel_text(msg_raw) and s.get("stage") in ("payment", "choose_payment", "add_more", "cancel_choice", "cancel_item_select"):
        ...
        if is_entire_cancel:
            ...
            reply = "تم إلغاء طلبك بالكامل." if lang == "ar" else "Your entire order has been cancelled."
            return make_chat_response(reply, lang)
    ...
    fresh_session = get_session(user_number, lang)
