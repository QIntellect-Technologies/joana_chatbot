...
def handle_multi_item_text(msg_raw, s, MENU, lang):
    ...
    # ✅ If nothing to process, show error
    if not added_lines and not generics:
        msg = (
...
def canonical_category(raw_category: str, name_en: str = "") -> str:
    ...
    if c in ("drinks", "drink", "beverages", "beverage"):
        return "drinks"
    if c in ("burgers", "burger", "burgers & meals", "burgers and meals"):
    ...
    # 2. Fallback Inference from Name
    ...
    if " meal" in n or n.endswith("meal"):
        return "meals"
    if "wrap" in n: # Extra safety for wraps
        return "sandwiches"
...
def parse_intelligent_order(msg: str, lang: str = "en") -> dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            "RULES:\n"
            "1. If user says generic category (\"burger\", \"sandwich\", \"drink\", \"meal\", \"juice\") → type=\"generic\"\n"
            "2. If user mentions SPECIFIC item name (\"Beef Burger\", \"Coffee\", \"Pepsi\") → type=\"specific\"\n"
            "3. Handle typos intelligently: cofee→Coffee, burgur→Burger, sandwihc→Sandwich\n"
