...
def update_order_payment(order_id: int | None, method: str, status: str) -> None:
    ...
    payload = {
        ...
        "payment_time_local": ts["local_timestamp"],
    }
    try:
        resp = requests.patch(url, headers=supabase_headers("return=minimal"), json=payload, timeout=20)
...
def extract_items_with_llm(msg: str, lang: str = "en") -> list:
    ...
    try:
        ...
        system_prompt = (
            "You are an order-extraction engine for JOANA Fast Food.\n"
            "Extract items from the restaurant menu.\n"
            "Customer may write in Arabic, English or Roman Urdu.\n\n"
            f"- MENU ITEMS (ENGLISH NAMES, do NOT invent others): {menu_list}\n\n"
            "Return ONLY valid JSON:\n"
            "{\"items\":[{\"name\":\"ITEM_NAME_FROM_MENU\",\"qty\":NUMBER,\"spicy\":\"spicy/non-spicy/any\"}]}\n\n"
            "Rules:\n"
            "- 'name' MUST be an English menu item exactly from MENU ITEMS.\n"
            "- qty default 1.\n"
            "- include all items.\n"
            "- spicy: spicy / non-spicy / any.\n"
            "- JSON only."
        )
        res = client.chat.completions.create(
            model="gpt-4o-mini",
