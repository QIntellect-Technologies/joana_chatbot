...
def whatsapp_webhook():
    ...
    if not from_button:  # Only for text messages, not button clicks
        ...
        new_order_keywords = [
            "new order", "start new order", "another order", "add one more order", "start again",
            "order more", "want to order more", "i want to order", "order again", "place new order",
            "طلب جديد", "طلب آخر", "أبدأ من جديد", "طلب جديد من فضلك", "أريد طلب", "طلب مرة أخرى"
        ]
        if any(keyword in user_text_lower for keyword in new_order_keywords):
