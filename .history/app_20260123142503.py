...
def whatsapp_webhook():
    ...
    # --- SESSION RESET: If session is missing, user cleared chat, or previous order is complete, start new order ---
    def start_new_order_session(lang):
        ...
        return WHATSAPP_SESSIONS[user_number]

    # If session is missing or user cleared chat, always start a new order
    if not ctx or not ctx.get("state") or ctx.get("state", {}).get("order") is None:
        lang = stored_lang if stored_lang in ("en", "ar") else "en"
        ctx = start_new_order_session(lang)

    # If previous order is complete (after payment), always start a new order
    if ctx.get("state", {}).get("stage") == "order_complete":
