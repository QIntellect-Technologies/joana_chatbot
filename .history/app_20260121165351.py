...
def chat():
    ...
    # ✅ DEBUG: Log initial state for debugging multi-item issues
    print(f"🔍 DEBUG [chat start]: msg='{msg[:50]}...', stage={s.get('stage')}, from_button={from_button}")
    ...
    # =====================================================
    # ⚠️ WARNING: This OLD code intercepts "4 burgers and 2 drinks" and only processes burgers
    # It should ONLY run if multi-item handler didn't trigger
    # Check if this is truly a single-item burger order (no "and", no other quantities)
    ...
    # ✅ CRITICAL FIX: Don't use old burger detection if this looks like multi-item
    if looks_like_multi_item_text(msg_raw):
        print(f"⚠️ DEBUG: Skipping old burger detection because looks_like_multi_item=True")
        # Skip to avoid intercepting multi-item orders
    elif (has_generic_burg or has_generic_sand) and has_other_menu_item:
    ...
    elif msg_has_generic_burger_word(msg) and not msg_has_specific_burger(msg):
        print(f"🔍 DEBUG [old burger detection]: Triggering for msg='{msg}'")
        qty_guess = extract_qty_for_generic(msg, "burger")
        s["stage"] = "await_specific_burger"
        s["last_qty"] = qty_guess
        s["last_item"] = None
    ...
    elif msg_has_generic_sandwich_word(msg) and not msg_has_specific_sandwich(msg):
        qty_guess = extract_qty_for_generic(msg, "sandwich")
        s["stage"] = "await_specific_sandwich"
        s["last_qty"] = qty_guess
        s["last_item"] = None
