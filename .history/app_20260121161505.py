...
def parse_intelligent_order(msg: str, lang: str = "en") -> dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            "1. **WRAPS = SANDWICHES (MANDATORY)**: When user says 'wrap', 'wraps', 'tortilla' → MUST use category=\"sandwich\"\n"
            "   ⚠️ NEVER ignore wraps! ALWAYS extract them!\n"
            "   Examples:\n"
            "   - Input: '3 burgers and 2 wraps' → Output: [{\"burger\": 3}, {\"sandwich\": 2}] ← TWO separate items!\n"
            "   - Input: '2 wraps' → {\"type\": \"generic\", \"category\": \"sandwich\", \"qty\": 2}\n"
            "   - Input: '5 ساندويتش' → {\"type\": \"generic\", \"category\": \"sandwich\", \"qty\": 5}\n"
            "2. **SIDES/SNACKS**: 'sides', 'snacks', 'side' → category=\"snacks_sides\"\n"
            "   Example: '5 sides' → {\"type\": \"generic\", \"category\": \"snacks_sides\", \"qty\": 5}\n"
            "3. **ARABIC NUMBERS**: Convert Arabic numerals (١٢٣٤٥) to English (12345)\n"
            "4. Generic categories (English OR Arabic):\n"
            "   - burger/burgers/برجر → type=\"generic\", category=\"burger\"\n"
            "   - sandwich/ساندويتش/wrap/wraps → type=\"generic\", category=\"sandwich\"\n"
            "   - drink/مشروب/juice/عصير/meal/وجبة → respective categories\n"
            "5. If user mentions SPECIFIC item name (partial/full, English/Arabic) → type=\"specific\"\n"
            "   Examples: 'chicken' OR 'دجاج' → \"Chicken Burger\", 'sweet corn' OR 'ذرة حلوة' → \"Sweet Corn\"\n"
            "6. MATCHING RULE: \"2 chicken\" or \"٢ دجاج\" → SPECIFIC item \"Chicken Burger\", NOT generic\n"
            "7. Handle typos: cofee→Coffee, burgur→Burger, wrps→wraps\n"
            "8. Extract ALL items from multi-item orders (CRITICAL - DO NOT DROP ANY!):\n"
            "   - Input: \"3 burgers and 2 wraps and 4 coffee\" → Output: [burger(3), sandwich(2), coffee(4)] ← THREE items!\n"
            "   - Input: \"5 burgers and 5 sides\" → Output: [burger(5), snacks_sides(5)]\n"
            "   - Input: \"٤ برجر و ٥ ساندويتش و ٣ ذرة\" → Output: [burger(4), sandwich(5), sweet_corn(3)]\n"
            "9. **CRITICAL**: PRESERVE EXACT QUANTITIES from user input!\n"
            "10. Detect preferences: spicy/non-spicy/حار/بدون حار for burgers\n\n"
            "OUTPUT (JSON ONLY):\n"
            "{\n"
            "  \"items\": [\n"
            ...
            "  ]\n"
            "}\n\n"
            "- category: burgers_meals, sandwiches, drinks, meals, juices, snacks_sides\n"
            "- name: exact English menu item name\n"
            "- For wraps/tortillas/ساندويتش → category=\"sandwich\"\n"
...
def chat():
    ...
    elif s.get("order") and any(k in msg_txt.lower() for k in ["place order", "place the order", "complete", "finish", "checkout", "check out", "payment", "pay", "done", "proceed", "cash", "online"]):
        summary, total = build_order_summary_and_total(s.get("order") or [], lang)
        s["total"] = total
    ...
    if s.get("stage") == "choose_payment":
        if any(x in msg_l for x in ["cash", "cod"]) or any(x in msg for x in ["نقد", "نقدا"]):
            ...
            receipt = (
                ...
                f"<b>طريقة الدفع:</b> نقدي<br><br>"
                "شكراً لطلبك! سيكون جاهزاً خلال 20 إلى 30 دقيقة."
                if lang == "ar" else
                "✅ <b>ORDER RECEIPT</b><br><br>"
                "<b>Order:</b><br>" + "<br>".join(summary) + "<br><br>"
                f"<b>Total:</b> {total:.2f} {CURRENCY}<br>"
                "<b>Payment Method:</b> Cash<br><br>"
                "Thank you! Your order will be ready in 20 to 30 minutes."
            )
        ...
        if any(x in msg_l for x in ["online", "online payment", "pay online", "card", "visa", "master", "debit", "mada"]) or any(x in msg for x in ["مدى", "إلكتروني", "دفع", "كرت"]):
            ...
            # ✅ BUILD RECEIPT with payment link
            summary, _ = build_order_summary_and_total(s.get("order") or [], lang)
            reply = (
                "✅ <b>إيصال الطلب</b><br><br>"
                "<b>الطلب:</b><br>" + "<br>".join(summary) + "<br><br>"
                f"<b>الإجمالي:</b> {total:.2f} {CURRENCY_AR}<br>"
                "<b>طريقة الدفع:</b> أونلاين<br><br>"
                f"<a href='{pay_url}' target='_blank'><b>اضغط هنا لإتمام الدفع</b></a>"
                if lang == "ar" else
                "✅ <b>ORDER RECEIPT</b><br><br>"
                "<b>Order:</b><br>" + "<br>".join(summary) + "<br><br>"
                f"<b>Total:</b> {total:.2f} {CURRENCY}<br>"
                "<b>Payment Method:</b> Online<br><br>"
                f"<a href='{pay_url}' target='_blank'><b>Click here to complete your payment</b></a>"
            )
            session["state"] = {"stage": None, "order": [], "total": 0, "last_item": None, "last_qty": 0, "last_confirmed_item": None, "pending_item": None, "spice_queue": [], "generic_queue": []}
            return make_chat_response(reply, lang)
