...
def correct_arabic_typos_with_ai(msg: str) -> str:
    ...
    try:
        ...
        # Safety: Only use correction if it's reasonable (not empty, similar length)
        if corrected and len(corrected) >= len(msg) * 0.5 and len(corrected) <= len(msg) * 2:
            print(f"🔧 Arabic typo correction: '{msg}' → '{corrected}'")
            return corrected
        else:
            print(f"⚠️ Arabic correction skipped (unreasonable output): '{corrected}'")
            return msg
